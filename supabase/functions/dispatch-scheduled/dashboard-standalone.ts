...
// send-pipeline code so it can be pasted as the entire contents of a single
// "index.ts" with nothing else to add.
//
// HOW TO DEPLOY THIS VIA THE DASHBOARD:
//   1. Edge Functions -> Create a new function.
//   2. Function name: dispatch-scheduled   (must be exactly this — the
//      pg_cron job calls it by this name; see supabase.sql).
...
//        DISPATCH_SCHEDULED_SECRET  = any random string you make up — this
//                                     is the shared secret pg_cron sends as
//                                     a Bearer token; it does NOT need to
//                                     match anything else.
//        APP_ORIGIN                 = your production URL
//        FCM_SERVICE_ACCOUNT_JSON   = same value as send-notification's
//   6. Only useful once you've also enabled pg_cron + pg_net (Database ->
//      Extensions) and run the cron.schedule(...) block documented at the
//      bottom of supabase.sql with this function's URL and the same
//      DISPATCH_SCHEDULED_SECRET. Skip this whole function for now if you
//      don't need scheduled (future-dated) sends yet — "Send Now" only
...
// ---------------------------------------------------------------------------
// HTTP entrypoint. Invoked on a schedule by pg_cron, not a browser — the
// caller proves itself with a shared secret compared against the
// Authorization header, not a user session.
// ---------------------------------------------------------------------------
